...
export const toggleAcolyteOfNatureCantripBtnStyle = makeStyles(() => ({
  addButton: {
    fontSize: "12px",
    padding: "2px 10px",
    borderRadius: "4px",
...
export const PrepareAcolyteOfNatureCantripButton = ({ spell, index = 0 }) => {
  ...
  const toggle = () => {
    ...
    setCharacterInfo((prev) => {
      const current = prev?.acolyteOfNatureCantrip || null;
      ...
      return { ...prev, acolyteOfNatureCantrip: next };
    });
  };

  const classes = toggleAcolyteOfNatureCantripBtnStyle();
...
export default PrepareAcolyteOfNatureCantripButton;
