...
export const toggleArcaneMasteryBtnStyle = makeStyles(() => ({
  addButton: {
    fontSize: "12px",
    padding: "2px 10px",
    borderRadius: "4px",
...
export const PrepareArcaneMasterySpellButton = ({ spell, index = 0 }) => {
  ...
  const toggle = () => {
    ...
    setCharacterInfo((prev) => {
      const current = Array.isArray(prev?.arcaneMasterySpells) ? prev.arcaneMasterySpells : [];
      ...
      return { ...prev, arcaneMasterySpells: next };
    });
  };

  const classes = toggleArcaneMasteryBtnStyle();
...
export default PrepareArcaneMasterySpellButton;
