...
export const toggleArcaneArcherLoreCantripBtnStyle = makeStyles(() => ({
  addButton: {
    fontSize: "12px",
    padding: "2px 10px",
    borderRadius: "4px",
...
export const PrepareArcaneArcherLoreCantripButton = ({ spell, index = 0 }) => {
  ...
  const toggle = () => {
    ...
    setCharacterInfo((prev) => {
      const current = prev?.arcaneArcherLoreCantrip || null;
      ...
      return { ...prev, arcaneArcherLoreCantrip: next };
    });
  };

  const classes = toggleArcaneArcherLoreCantripBtnStyle();
...
export default PrepareArcaneArcherLoreCantripButton;
